...
public class MainPage {
    ...
    public static final String YANDEX_HOMEPAGE = "https://ya.ru/"; // URL главной страницы Яндекса
    private static final String FAQ_QUESTIONS_ELEMENT = "accordion__heading-"; // часть локатора вопроса + нужно добавлять индекс в методе для выбора конкретного вопроса
    private static final String FAQ_ANSWER_ELEMENT = "accordion__panel-"; // часть локатора ответа + нужно добавлять индекс в методе для выбора конкретного ответа
    ...
    public WebElement getQuestionElement(int index) {
        return driver.findElement(By.id(FAQ_QUESTIONS_ELEMENT + index));
    }
    ...
    public String getAnswerTextByIndex(int index) {
        new WebDriverWait(driver, Duration.ofSeconds(3))
                .until(ExpectedConditions.visibilityOfElementLocated(By.id(FAQ_ANSWER_ELEMENT + index))); // Ждет, пока элемент станет видимым
        return driver.findElement(By.id(FAQ_ANSWER_ELEMENT + index)).getText();
    }
